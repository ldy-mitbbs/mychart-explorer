...
export default function Encounters() {
  ...
  return (
    <>
      <h1>Encounters <span className="muted small">({rows.length})</span></h1>
      <input
        placeholder="filter by provider / department…"
        value={filter}
        ...
        {selected && detail && (
          <div className="card" style={{ maxHeight: 'calc(100vh - 200px)', overflowY: 'auto' }}>
            <div className="row" style={{ justifyContent: 'space-between' }}>
              <h2 style={{ margin: 0, border: 0 }}>Encounter {selected}</h2>
            ...
            {detail.reasons?.length > 0 && (
              <>
                <h3 style={{ fontSize: 13 }}>Reasons for visit</h3>
                <ul className="small">
            ...
            {detail.diagnoses?.length > 0 && (
              <>
                <h3 style={{ fontSize: 13 }}>Diagnoses</h3>
                <ul className="small">
                  {detail.diagnoses.map((d: any, i: number) => (
                    <li key={i}>{d.DX_ID_DX_NAME}{d.PRIMARY_DX_YN === 'Y' && <em className="muted"> (primary)</em>}</li>
                  ))}
                </ul>
              </>
            )}
            ...
            {detail.meds?.length > 0 && (
              <>
                <h3 style={{ fontSize: 13 }}>Medication orders ({detail.meds.length})</h3>
                <ul className="small">
            ...
            {detail.orders?.length > 0 && (
              <>
                <h3 style={{ fontSize: 13 }}>Orders ({detail.orders.length})</h3>
                <ul className="small">
            ...
            <details>
              <summary>Raw encounter row</summary>
